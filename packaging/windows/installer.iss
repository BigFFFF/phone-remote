...
Filename: "{sys}\netsh.exe"; Parameters: "advfirewall firewall add rule name=""{#ApiRuleName}"" dir=in action=allow program=""{app}\{#MyAppExeName}"" protocol=TCP localport=8765 profile=private remoteip=LocalSubnet enable=yes"; Flags: runhidden; StatusMsg: "Configuring the Private-network API firewall rule..."
